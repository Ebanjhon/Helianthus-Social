export const accountNew = [
  {
    avatar:
      'https://i.pinimg.com/736x/dd/c2/af/ddc2af28b94c2edcca3ab2775120d12f.jpg',
    username: 'Tuấn MP',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=2',
    username: 'Thành Ngân',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=3',
    username: 'michael Trân',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=4',
    username: 'Sam Nguyễn',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=5',
    username: 'Henry Nguyễn',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=6',
    username: 'dwight_schrute',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=7',
    username: 'andy_bernard',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=8',
    username: 'angela_martin',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=9',
    username: 'kevin_malone',
    isFollow: false,
  },
  {
    avatar: 'https://i.pravatar.cc/150?img=10',
    username: 'stanley_hudson',
    isFollow: false,
  },
];

export const feeds = [
  {
    media: [
      {
        id: '2',
        url: 'https://i.pinimg.com/736x/a7/fc/6f/a7fc6fec8d9305423018e54bada02dbf.jpg',
        width: 700,
        height: 500,
        typeMedia: 'IMAGE',
      },
      {
        id: '3',
        url: 'https://storage.googleapis.com/myproject0810/uploads/1743150677240-264272_tiny.mp4?GoogleAccessId=firebase-adminsdk-fbsvc%40my-project-c7f44.iam.gserviceaccount.com&Expires=1898528400&Signature=HLJlIbVi3HTnlPBVbtYN1V0wv0deKRPHK88eBpUYpVMV8WeNEwll1MUUMapZs%2BoiNj08S54R5%2B%2FBixJL6CulMIcQNsL8S1PcvnVhgbbRMiBtC7UQhQpkzw1TbxLvJN9XKa371xOD5Hfxyo1jZD0Rwl5Nld0EsY6ivrVlrR0BlQktdjIvitijN1xZY6eHFfC%2B9vU7VekHroP8BdfFWxvwoPwVOHWk7k6kP%2Bi75RBm39CoLkvdv5vYO3JBv4vZipVYvJxzUaYYk8mTLJ3xcs%2FA6B1DUXo5zsy1r7g7NrJJctJPGU70KzdI33PmsBSikaJ%2Fcc0wBUv6CdSzQ1xTNRxpnA%3D%3D',
        width: 1280,
        height: 720,
        typeMedia: 'VIDEO',
      },
    ],
    username: 'jane_smith',
    firstname: 'Jane',
    lastname: 'Smith',
    timecreate: '2025-04-16T14:30:00Z',
    avatar:
      'https://i.pinimg.com/736x/a7/24/d3/a724d352978a010ceea8ed4ad8d41fd4.jpg',
    countLike: 85,
    countComment: 10,
    isLike: true,
    isFollow: true,
    isCurrentUser: true,
    content: 'Had a great time hiking this weekend!',
  },
  {
    media: [
      {
        id: '4',
        url: 'https://i.pinimg.com/736x/47/a5/0c/47a50cc93073809c771624ffb6d96942.jpg',
        width: 900,
        height: 400,
        typeMedia: 'IMAGE',
      },
    ],
    username: 'michael_scott',
    firstname: 'Michael',
    lastname: 'Scott',
    timecreate: '2025-04-15T10:15:00Z',
    avatar: 'https://i.pravatar.cc/150?img=3',
    countLike: 200,
    countComment: 25,
    isLike: false,
    isFollow: true,
    isCurrentUser: false,
    content: 'Just closed a big deal at work!',
  },
  {
    media: [
      {
        id: '5',
        url: 'https://i.pinimg.com/736x/b0/22/84/b022844644587cc2f7a6b5a34cb1ceb7.jpg',
        width: 900,
        height: 400,
        typeMedia: 'IMAGE',
      },
    ],
    username: 'pam_beesly',
    firstname: 'Pam',
    lastname: 'Beesly',
    timecreate: '2025-04-14T09:45:00Z',
    avatar:
      'https://i.pinimg.com/736x/49/2d/5c/492d5c6f0b1ed1ada40139603d1eb702.jpg',
    countLike: 150,
    countComment: 20,
    isLike: true,
    isFollow: false,
    isCurrentUser: false,
    content: 'Loving the new art exhibit downtown.',
  },
  {
    media: [
      {
        id: '6',
        url: 'https://i.pinimg.com/736x/28/6d/6c/286d6cc0c7f3eb7860f53bdd86f2af64.jpg',
        width: 800,
        height: 600,
        typeMedia: 'IMAGE',
      },
    ],
    username: 'jim_halpert',
    firstname: 'Jim',
    lastname: 'Halpert',
    timecreate: '2025-04-13T16:20:00Z',
    avatar: 'https://i.pravatar.cc/150?img=5',
    countLike: 95,
    countComment: 12,
    isLike: false,
    isFollow: true,
    isCurrentUser: false,
    content: 'Weekend getaway with friends!',
  },
];
